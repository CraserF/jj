...
pub struct DominatorFinder<'a, N> {
    ...
    immediate_dominators: Vec<Index>,
}
    ...
    N: Clone + Eq + Hash,
{
        ...
        NI: IntoIterator<Item = N>,
    {
        ...
        let target_ids: Vec<Index> = target_set
            .into_iter()
    ...
    fn calculate_immediate_dominators(rev_adj: &[Vec<Index>]) -> Vec<Index> {
        ...
        // index.
        let mut immediate_dominators: Vec<Index> = vec![usize::MAX; num_nodes];
    ...
    // See http://www.hipersoft.rice.edu/grads/publications/dom14.pdf for details on how this function works.
    fn intersect(mut b1: Index, mut b2: Index, immediate_dominators: &[Index]) -> Index {
        while b1 != b2 {
            while b1 < b2 {
    ...
    // See http://www.hipersoft.rice.edu/grads/publications/dom14.pdf for details on how this function works.
    fn find_lowest_common_ancestor(targets: &[Index], immediate_dominators: &[Index]) -> Index {
    ...
    N: Clone + Eq + Hash,
{
    ...
    where
        V: Eq + Hash,
    {
        let mut value_adj: IndexMap<&'a V, IndexSet<&'a V>> = IndexMap::new();
        let mut seen_value_edge = HashSet::new();
        ...
        for (parent, children) in &self.graph.adj {
            ...
            let value_adj_entry = value_adj.entry(parent_value).or_default();
            seen_children.clear();
            for child in children {
                if seen_children.insert(child) {
                    let child_value = node_values.get(child).expect("cached");
                    if seen_value_edge.insert((parent_value, child_value)) {
                        value_adj_entry.insert(child_value);
                    }
                }
            }
        }
        FlowGraph::new(SimpleDirectedGraph::new(value_adj), start_value)
    }
...
mod tests {
    ...
    fn test_find_dominator_value_with_unknown_node_in_target_set() {
        // Flow graph: A(1) -> B(2).
        let simple_graph = SimpleDirectedGraph::from_edge_list([("A", "B")]);
        let flow_graph = FlowGraph::new(simple_graph, "A");
        let value_fn = async |node: &&str| match *node {
            "A" => Ok(1),
            "B" => Ok(2),
